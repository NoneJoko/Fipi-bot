...
# Функция для отправки клавиатуры с заданиями и кнопкой /back
def send_tasks_keyboard(message, subject):
    markup = ReplyKeyboardMarkup(resize_keyboard=True)
    nums = [str(i) for i in range(1, 31)]
    for i in range(0, 30, 5):
        markup.row(*nums[i:i+5])
    markup.row('/back')  # добавляем кнопку "назад" как 31-ю
    bot.send_message(
        message.chat.id,
        f"Вы выбрали «{subject}».\nВыберите номер задания (1–30) или нажмите /back для возврата:",
        reply_markup=markup
    )

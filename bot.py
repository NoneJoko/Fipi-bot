...
variants = {
    'Русский язык': {
        'Вариант 1': {
            ...
            '9': {
                'question': text_part2 + "\n\nЗадание 9. Среди предложений 7–12 найдите предложение с обособленным обстоятельством. Напишите номер этого предложения или 'нет', если такого нет.",
...
def check_answer(user_answer_raw: str, correct):
    ...
    if re.fullmatch(r'\d', str(correct)):
        ...
        return num == str(correct), str(correct)
    return ua == normalize_text(str(correct)), str(correct)
...
def task1_playback_sequence(chat_id, voice_text):
    ...
    threading.Timer(300, send_second).start()
...
def on_choose_task(message):
    ...
    if task_num == '1':
        ...
        bot.send_message(chat_id, "Введите ваш ответ (для вариантов — 1), 2), 3), 4)):")
